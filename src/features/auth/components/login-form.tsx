    ...
    <form onSubmit={handleSubmit} className="w-full space-y-3.5">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className="w-full rounded-xl border border-brand-line bg-white px-4 py-2.5 text-sm text-brand-text outline-none transition focus:border-brand-forest focus:ring-2 focus:ring-brand-forest/20 disabled:cursor-not-allowed disabled:opacity-60"
      ...
      <div className="space-y-2">
        <label
          htmlFor="password"
          className="text-xs font-semibold uppercase tracking-wider text-brand-text-soft"
        >
          Kata Sandi
        </label>
        <input
      ...
      <button
        ...
        className="relative flex w-full items-center justify-center gap-2 rounded-xl bg-brand-ink px-4 py-3 text-sm font-bold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-70 shadow-sm mt-2"
      >
        {isSubmitting && (
          <svg
      ...
      <p className="text-center text-sm text-brand-text-soft">
        Belum punya akun?{" "}
        <Link href="/register" className="font-bold text-brand-text hover:underline">
          Daftar
        </Link>
      </p>
      ...
      {/* Keep the Google button mounted at all times (so the SDK-rendered
          button survives a failed attempt); overlay a centered spinner while
          connecting instead of swapping in a separate loading row. */}
